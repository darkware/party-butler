package partybutler.ui.customcomponents.coverflow;
...
public class CDShelf extends JPanel{
    ...
    public CDShelf(ArrayList<Album> albums) {
        avatarFont = new Font("Walkway Black", Font.PLAIN, 24);
...
//    }
    ...
    /**
     * Will be invoked on Avatar Selection (Click)
     * @param listener
     */
    ...
    private void notifyActionListeners(){
        for(ActionListener listener : actionListeners){
            listener.actionPerformed(new ActionEvent(this,0,"Avatar Pressed"));
        }
    }
    ...
    private void notifyListSelectionListener()
    {
        ...
        System.out.println("Avatar Changed to: "+albums.get(avatarIndex).getName());
    ...
    private class PicturesFinderThread implements Runnable {
        ...
        public void run() {
            ...
            for (Album album: albums) {
                try {
                    BufferedImage image = album.getImage();

                    avatars.add(fx.createReflectedPicture(fx.createCrystalCase(image)));
                    avatarsText.add(album.getName());
                } catch (Exception e) {
                    e.printStackTrace();
                }
                
                if (++i == albums.size()) {
                    setAvatarIndex(albums.size() / 2);
                    startFader();
                }
            }
    ...
    public Album getSelectedAlbum(){
        return albums.get(getSelectedIndex());
    }
